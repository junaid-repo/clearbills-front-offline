...
const CustomersPage = ({ setSelectedPage }) => {
    ...
    // --- CHANGE 1: Dynamic Page Size State ---
    ...
    const fetchCustomers = useCallback(async (page = 1) => {
        ...
        // --- CHANGE 2 (cont): Added pageSize to dependency array
    }, [apiUrl, debouncedSearchTerm, sortConfig, pageSize]);
    ...
    // --- CHANGE 3: Handle "Select All" Logic ---
    const handleSelectAll = (e) => {
        const isChecked = e.target.checked;
        const newSelection = new Set(selectedCustomers);

        if (isChecked) {
            // Add all visible customers
            customers.forEach(c => newSelection.add(c.id));
        } else {
            // Remove all visible customers
    ...
    // --- CHANGE 4: Handler for Page Size Change ---
    ...
    // Helper to check if all visible items are selected
    const isAllVisibleSelected = customers.length > 0 && customers.every(c => selectedCustomers.has(c.id));
    ...
    return (
        <div className="page-container">
            ...
            {/* --- CHANGE 6: Updated Pagination Section with Page Selector --- */}
            {customers.length > 0 && (
